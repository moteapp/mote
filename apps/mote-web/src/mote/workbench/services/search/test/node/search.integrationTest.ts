...
import { IFolderQuery, QueryType, IRawFileMatch } from 'mote/workbench/services/search/common/search';
import { Engine as FileSearchEngine, FileWalker } from 'mote/workbench/services/search/node/fileSearch';
import { flakySuite } from 'vs/base/test/node/testUtils';
import { FileAccess } from 'vs/base/common/network';

const TEST_FIXTURES = path.normalize(FileAccess.asFileUri('mote/workbench/services/search/test/node/fixtures').fsPath);
...
const ROOT_FOLDER_QUERY_36438: IFolderQuery[] = [
	{ folder: URI.file(path.normalize(FileAccess.asFileUri('mote/workbench/services/search/test/node/fixtures2/36438').fsPath)) }
];
...
flakySuite('FileSearchEngine', () => {
	...
	test('Files: extraFiles only', function (done: () => void) {
		const engine = new FileSearchEngine({
			...
			folderQueries: [],
			extraFileResources: [
				URI.file(path.normalize(path.join(FileAccess.asFileUri('mote/workbench/services/search/test/node/fixtures').fsPath, 'site.css'))),
				URI.file(path.normalize(path.join(FileAccess.asFileUri('mote/workbench/services/search/test/node/fixtures').fsPath, 'examples', 'company.js'))),
				URI.file(path.normalize(path.join(FileAccess.asFileUri('mote/workbench/services/search/test/node/fixtures').fsPath, 'index.html')))
			],
			filePattern: '*.js'
		});
