...
export interface IWorkbenchContributionsRegistry {
	 ...
	 */
	registerWorkbenchContribution<Services extends BrandedService[]>(id: string, ctor: IWorkbenchContributionSignature<Services>, instantiation: WorkbenchContributionInstantiation): void;
...
export class WorkbenchContributionsRegistry extends Disposable implements IWorkbenchContributionsRegistry {
	...
	start(accessor: ServicesAccessor): void {
		...
		for (const editorTypeId of this.contributionsByEditor.keys()) {
			if (editorPaneService.didInstantiateEditorPane(editorTypeId)) {
				this.onEditor(editorTypeId, instantiationService, lifecycleService, logService);
			}
		}
		this._register(editorPaneService.onWillInstantiateEditorPane(e => this.onEditor(e.typeId, instantiationService, lifecycleService, logService)));
	}

	private onEditor(editorTypeId: string, instantiationService: IInstantiationService, lifecycleService: ILifecycleService, logService: ILogService): void {
		const contributions = this.contributionsByEditor.get(editorTypeId);
		if (contributions) {
			this.contributionsByEditor.delete(editorTypeId);

			for (const contribution of contributions) {
				this.safeCreateContribution(instantiationService, logService, contribution, lifecycleService.phase);
			}
		}
	}
	...
	registerWorkbenchContribution(id: string, ctor: IConstructorSignature<IWorkbenchContribution>, phase: WorkbenchPhase.BlockStartup | WorkbenchPhase.BlockRestore): void;
	registerWorkbenchContribution(id: string | undefined, ctor: IConstructorSignature<IWorkbenchContribution>, phase: WorkbenchPhase.AfterRestored | WorkbenchPhase.Eventually): void;
	registerWorkbenchContribution(id: string, ctor: IConstructorSignature<IWorkbenchContribution>, lazy: ILazyWorkbenchContributionInstantiation): void;
	registerWorkbenchContribution(id: string, ctor: IConstructorSignature<IWorkbenchContribution>, onEditor: IOnEditorWorkbenchContributionInstantiation): void;
	registerWorkbenchContribution(id: string | undefined, ctor: IConstructorSignature<IWorkbenchContribution>, instantiation: WorkbenchContributionInstantiation): void {
		...
		if (
			this.instantiationService && this.lifecycleService && this.logService && this.editorPaneService &&
			(
				(typeof instantiation === 'number' && this.lifecycleService.phase >= instantiation) ||
				(typeof id === 'string' && isOnEditorWorkbenchContributionInstantiation(instantiation))
			)
		) {
			this.safeCreateContribution(this.instantiationService, this.logService, contribution, typeof instantiation === 'number' ? toLifecyclePhase(instantiation) : this.lifecycleService.phase);
		}
	...
	private safeCreateContribution(instantiationService: IInstantiationService, logService: ILogService, contribution: IWorkbenchContributionRegistration, phase: LifecyclePhase): void {
		...
		try {
			if (typeof contribution.id === 'string') {
				mark(`mote/willCreateWorkbenchContribution/${phase}/${contribution.id}`);
			}
			...
			if (typeof contribution.id === 'string') {
				mark(`mote/didCreateWorkbenchContribution/${phase}/${contribution.id}`);
			}
		}
	...
	private async doInstantiateByPhase(instantiationService: IInstantiationService, logService: ILogService, phase: LifecyclePhase): Promise<void> {
		...
		if (contributions) {
			...
			switch (phase) {
				...
				case LifecyclePhase.Ready: {
					...
					for (const contribution of contributions) {
						this.safeCreateContribution(instantiationService, logService, contribution, phase);
					}
	...
	private doInstantiateWhenIdle(contributions: IWorkbenchContributionRegistration[], instantiationService: IInstantiationService, logService: ILogService, phase: LifecyclePhase): void {
		mark(`code/willCreateWorkbenchContributions/${phase}`);
 ...
 */
export const registerWorkbenchContribution = WorkbenchContributionsRegistry.INSTANCE.registerWorkbenchContribution.bind(WorkbenchContributionsRegistry.INSTANCE) as {
...
Registry.add(WorkbenchExtensions.Workbench, WorkbenchContributionsRegistry.INSTANCE);
