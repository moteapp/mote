...
import { EditorInputCapabilities, GroupIdentifier, isResourceEditorInput, IUntypedEditorInput, pathsToEditors } from 'mote/workbench/common/editor';
import { SidebarPart } from 'mote/workbench/browser/parts/sidebar/sidebarPart';
import { PanelPart } from 'mote/workbench/browser/parts/panel/panelPart';
import { Position, Parts, PanelOpensMaximizedOptions, IWorkbenchLayoutService, positionFromString, positionToString, panelOpensMaximizedFromString, PanelAlignment, ActivityBarPosition, LayoutSettings, MULTI_WINDOW_PARTS, SINGLE_WINDOW_PARTS, ZenModeSettings, EditorTabsMode, EditorActionsLocation, shouldShowCustomTitleBar } from 'mote/workbench/services/layout/browser/layoutService';
...
import { StartupKind, ILifecycleService } from 'mote/workbench/services/lifecycle/common/lifecycle';
...
import { EditorGroupLayout, GroupsOrder, IEditorGroupsService } from 'mote/workbench/services/editor/common/editorGroupsService';
...
import { IBannerService } from 'mote/workbench/services/banner/browser/bannerService';
import { IPaneCompositePartService } from 'mote/workbench/services/panecomposite/browser/panecomposite';
import { AuxiliaryBarPart } from 'mote/workbench/browser/parts/auxiliarybar/auxiliaryBarPart';
import { ITelemetryService } from 'vs/platform/telemetry/common/telemetry';
import { IAuxiliaryWindowService } from 'mote/workbench/services/auxiliaryWindow/browser/auxiliaryWindowService';
...
export abstract class Layout extends Disposable implements IWorkbenchLayoutService {
	...
	protected createWorkbenchLayout(): void {
		...
		for (const part of [titleBar, editorPart, activityBar, sideBar, statusBar, auxiliaryBarPart, bannerPart]) {
	...
	private adjustPartPositions(sideBarPosition: Position, panelAlignment: PanelAlignment, panelPosition: Position): void {
		...
		const preMovePanelWidth = 0;//!this.isVisible(Parts.PANEL_PART) ? Sizing.Invisible(this.workbenchGrid.getViewCachedVisibleSize(this.panelPartView) ?? this.panelPartView.minimumWidth) : this.workbenchGrid.getViewSize(this.panelPartView).width;
		const preMovePanelHeight = 0;//!this.isVisible(Parts.PANEL_PART) ? Sizing.Invisible(this.workbenchGrid.getViewCachedVisibleSize(this.panelPartView) ?? this.panelPartView.minimumHeight) : this.workbenchGrid.getViewSize(this.panelPartView).height;
	...
	private arrangeMiddleSectionNodes(nodes: { editor: ISerializedNode; activityBar: ISerializedNode; sideBar: ISerializedNode; auxiliaryBar: ISerializedNode }, availableWidth: number, availableHeight: number): ISerializedNode[] {
		...
		const panelSize = 0; //this.stateModel.getInitializationValue(LayoutStateKeys.PANEL_SIZE) ? 0 : nodes.panel.size;
