...
import { AbstractIdleValue, IntervalTimer, TimeoutTimer, _runWhenIdle, IdleDeadline } from '@mote/base/common/async';
...
import { MoteWindow, ensureMoteWindow, mainWindow } from '@mote/base/browser/window';

export interface IRegisteredMoteWindow {
	readonly window: MoteWindow;
...
export const {
	...
	const windows = new Map<number, IRegisteredMoteWindow>();

	ensureMoteWindow(mainWindow, 1);
	const mainWindowRegistration = { window: mainWindow, disposables: new DisposableStore() };
	windows.set(mainWindow.moteWindowId, mainWindowRegistration);

	const onDidRegisterWindow = new event.Emitter<IRegisteredMoteWindow>();
	const onDidUnregisterWindow = new event.Emitter<MoteWindow>();
	const onWillUnregisterWindow = new event.Emitter<MoteWindow>();

	function getWindowById(windowId: number): IRegisteredMoteWindow | undefined;
	function getWindowById(windowId: number | undefined, fallbackToMain: true): IRegisteredMoteWindow;
	function getWindowById(windowId: number | undefined, fallbackToMain?: boolean): IRegisteredMoteWindow | undefined {
	...
	return {
		...
		registerWindow(window: MoteWindow): IDisposable {
			if (windows.has(window.moteWindowId)) {
			...
			windows.set(window.moteWindowId, registeredWindow);

			disposables.add(toDisposable(() => {
				windows.delete(window.moteWindowId);
		...
		getWindows(): Iterable<IRegisteredMoteWindow> {
		...
		getWindowById,
		getWindow(e: Node | UIEvent | undefined | null): MoteWindow {
			const candidateNode = e as Node | undefined | null;
			if (candidateNode?.ownerDocument?.defaultView) {
				return candidateNode.ownerDocument.defaultView.window as MoteWindow;
			}

			const candidateEvent = e as UIEvent | undefined | null;
			if (candidateEvent?.view) {
				return candidateEvent.view.window as MoteWindow;
			}
 ...
 * the main window.
 */
export function getActiveWindow(): MoteWindow {
	const document = getActiveDocument();
	return (document.defaultView?.window ?? mainWindow) as MoteWindow;
}
