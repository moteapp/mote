...
import { getWindowId, onDidUnregisterWindow } from '@mote/base/browser/dom';
...
class PixelRatioMonitorFacade {
	...
	private _getOrCreatePixelRatioMonitor(targetWindow: Window): PixelRatioMonitorImpl {
		...
		if (!pixelRatioMonitor) {
			...
			markAsSingleton(Event.once(onDidUnregisterWindow)(({ moteWindowId }) => {
				if (moteWindowId === targetWindowId) {
